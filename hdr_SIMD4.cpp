	...
	for( int i = 0; i < img_size; i += TILESIZE ){
		...
		for( int j = 0; j < pic_count; ++j ){
			...
			for( int k = 0; k < TILESIZE; ++k ){
				acc_E_debug[ i + k ] += w[ z[ k ] ] * ( g[ z[ k ] ] - ln_t[ j ] );
				acc_w_debug[ k ]     += w[ z[ k ] ];
			}
			/* debug end */
			...
			float32x4_t neon_temp_w    = vld1q_f32  ( temp_w );
			float32x4_t neon_temp_g    = vld1q_f32  ( temp_g );
			float32x4_t neon_temp_ln_t = vdupq_n_f32( ln_t[ j ] );

			neon_temp_g = vsubq_f32( neon_temp_g, neon_temp_ln_t );  // ( g[ z[ k ] ] - ln_t[ j ] )
			neon_temp_g = vmulq_f32( neon_temp_g, neon_temp_w );     // w[ z[ k ] ] * ( g[ z[ k ] ] - ln_t[ j ] )
			
			neon_acc_E  = vaddq_f32( neon_temp_g, neon_acc_E );      // acc_E[ i + k ] += w[ z[ k ] ] * ( g[ z[ k ] ] - ln_t[ j ] );
			neon_acc_w  = vaddq_f32( neon_temp_w, neon_acc_w );      // acc_w[ k ]     += w[ z[ k ] ];
			
			/* debug start
			bool correct = true;
			float debug[ TILESIZE ];
			vst1q_f32 ( debug, neon_acc_w );
			for( int k = 0; k < TILESIZE; ++k ){
				  correct = ( acc_w_debug[ k ] - debug[k] > 0.001 )? false : correct;
			}
			if( !correct ){
				cout << "ans : " << acc_w_debug[ 0 ] << " " << acc_w_debug[ 1 ] << " " << acc_w_debug[ 2 ] << " " << acc_w_debug[ 3 ] << endl;
				cout << "neon: " << debug[0] << " " << debug[1] << " " << debug[2] << " " << debug[3] << endl;
			}
			/* debug end */
